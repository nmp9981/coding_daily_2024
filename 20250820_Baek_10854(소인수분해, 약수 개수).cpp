...
//소인수 등록
map<ll,ll> EnrollFactorizationPrimes(vector<ll> factorizationPrimesList){
    map<ll,ll> numExp;//각 소인수의 지수
    for (int i = 0; i < factorizationPrimesList.size(); i++) {
        if(numExp.find(factorizationPrimesList[i]) != numExp.end()){//이미 해당 소인수 존재
            numExp[factorizationPrimesList[i]]+=1;
        }else{
            numExp[factorizationPrimesList[i]] =1;
        }
    }
...
//약수의 개수
void CountDivision(map<ll,ll> numExp){
    ll countOfDivision = 1;
    for (auto it = numExp.begin(); it != numExp.end(); ++it) {
        countOfDivision*=(it->second+1);
    }
    cout<<countOfDivision;
}
...
int main() {
    ...
    if(n==1){
        ...
        map<ll,ll> numExp = EnrollFactorizationPrimes(factorizationPrimesList);//각 소인수의 지수 등록
        CountDivision(numExp);//약수의 개수   
    }
    return 0;
}
