...
namespace Practice
{
    ...
    public class GJKPractice : MonoBehaviour
    {
        ...
        public bool GJK_Flow(List<Point> shapeA, List<Point> shapeB)
        {
            ...
            for(int i = 0; i < Iter; i++)
            {
                ...
                if (IsTriangleInOrigin(p1, p, p2))
                {
                    ...
                    //EPA
                    EPA_Flow(shapeA,shapeB, triangleList);
                    return true;
                }
                else
                {
        ...
        (float depth, Vector2 direction) EPA_Flow(List<Point> shapeA, List<Point> shapeB, List<Point> triangleList)
        {
            ...
            {
                //원점에서 가장 가까운 모서리
                Edge edge = FindClosestEdge(triangleList);

                //원점에서 normal방향으로 가장 멀리있는 점 찾기
            ...
            // 최대 반복 도달 — 마지막 결과 반환
            Edge final = FindClosestEdge(triangleList);
        ...
        {
            Vector2 ab = new Vector2(triangleList[1].x - triangleList[0].x, triangleList[1].y - triangleList[0].y);
            Vector2 bc = new Vector2(triangleList[2].x - triangleList[1].x, triangleList[2].y - triangleList[1].y);

            float ccw = CrossVector2(ab,bc);
            if (ccw >= 0)//반시계
            {
        ...
        /// <returns></returns>
        Edge FindClosestEdge(List<Point> triangleList)
        {
            ...
            {
                Point p = triangleList[i];
                Point p1 = triangleList[(i + 1)%count];

                Vector2 edgeVector = new Vector2(p1.x-p.x,p1.y-p.y);

                //방향과 거리
                Vector2 newNormal = new Vector2(edgeVector.y,-edgeVector.x).normalized;
                float curdist = p.x* newNormal.x+ p.y* newNormal.y;

                //더 가까운 거리
                if(curdist < closestEdge.distance)
                {
                    closestEdge.distance = curdist;
                    closestEdge.normal = newNormal;
                    closestEdge.index = i;
                }
            }
            return closestEdge;
